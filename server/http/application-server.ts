...
import {canonicalRequestPath,InvalidRequestPathError} from './request-path'
...
function send(response:ServerResponse,result:RuntimeResponse,head=false){response.writeHead(result.status,{...SECURITY_HEADERS,...result.headers});response.end(head?'':result.body)}
