...
function ResultCard({ product, onOpen, comparisonSelected, onToggleComparison }: { product: ProductCluster; onOpen: () => void; comparisonSelected:boolean; onToggleComparison:()=>void }) {
  const featured = product.featuredOfferId ? product.offers.find((offer) => offer.identity.provider === product.identity.provider && offer.identity.id === product.featuredOfferId) : undefined
...
function App() {
  ...
  async function search(nextQuery: string, nextCursor?: string) {
    ...
    if (!append) { setProducts([]); setComparisonIds([]); setCursor(undefined); setHasMore(false); setDetail(undefined); setDetailState('closed') }
    try { const result = await catalogClient.search({ query: nextQuery, country: 'GB', currency: 'GBP', limit: 6, cursor: nextCursor }); setProducts((current) => append ? [...current, ...result.products] : result.products); setProductEvidence(current=>refreshRetainedProductEvidence(current,result.products)); setCursor(result.pagination.cursor); setHasMore(result.pagination.hasMore); setStatus(!append && result.products.length === 0 ? 'empty' : 'success') }
