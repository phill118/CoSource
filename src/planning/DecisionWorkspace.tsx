...
export function DecisionWorkspace({ products, evidenceProducts, evidenceEntries, evidenceRefresh, plan, planEvaluation, scenarios, comparison, comparisonIds, pendingProposal, onClearComparison, onAddProduct, onRemoveLine, onSetQuantity, onSelectOffer, onRebase, onRefreshEvidence, onApplyScenario }: {
  ...
  pendingProposal:boolean;scenarios?:ScenarioSet;evidenceEntries:ProductEvidenceEntry[];evidenceRefresh:Record<string,EvidenceRefreshState>;onAddProduct:(identity:ProviderIdentity)=>void;onRemoveLine:(lineId:string)=>void;onSetQuantity:(lineId:string,quantity:number)=>void;onSelectOffer:(lineId:string,offer?:ProviderIdentity)=>void;onRebase:()=>void;onRefreshEvidence:(identity:ProviderIdentity)=>Promise<ApplicationResult<ProductEvidenceEntry>>;onApplyScenario:(input:unknown)=>ApplicationResult<PurchasePlan>
}) {
