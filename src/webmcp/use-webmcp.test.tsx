...
describe('WebMCP registration lifecycle',()=>{
  it('becomes ready only after all seven registrations succeed',async()=>{const signals:AbortSignal[]=[];const context=Object.assign(new EventTarget(),{async registerTool(_tool:WebMCPToolDefinition,options?:{signal?:AbortSignal}){signals.push(options!.signal!)}});Object.defineProperty(document,'modelContext',{configurable:true,value:context});const view=renderHook(()=>useWebMCP(state));await waitFor(()=>expect(view.result.current).toMatchObject({status:'ready',toolCount:7}));expect(signals).toHaveLength(7);view.unmount();expect(signals.every(signal=>signal.aborted)).toBe(true)})
