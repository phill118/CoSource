...
export function ProposalReviewPanel({proposals,goal,plan,products,onApprove,onReject}:{proposals:PlanChangeProposal[];goal:PurchaseGoal;plan:PurchasePlan;products:ProductCluster[];onApprove:(id:string)=>void;onReject:(id:string)=>void}){
  return <section className="proposal-review" aria-labelledby="proposal-title"><p className="eyebrow">4 · Human approval</p><h2 id="proposal-title">Agent proposals</h2><p>Agents can propose bounded plan changes. Only you can approve and apply them.</p>
    {proposals.length===0?<p className="muted">No agent proposals this session.</p>:proposals.map(proposal=>{const state={goal,plan,retainedProducts:products},status=proposalEffectiveStatus(proposal,state),preview=status==='pending'?applyProposalOperations(proposal,state):undefined
      return <article key={proposal.id} className={`proposal proposal-${status}`}><header><div><strong>Agent proposal</strong><h3>{proposal.id}</h3></div><span>{status}</span></header><p>{proposal.reason||'No reason supplied.'}</p><p>Current plan revision {plan.revision} · Target revision {proposal.expectedPlanRevision}</p>
        ...
        {status==='pending'&&preview?.ok&&<div className="proposal-preview"><section><h4>Before</h4>{evaluation(goal,plan,products)}</section><section><h4>After preview</h4>{evaluation(goal,preview.plan,products)}</section></div>}
