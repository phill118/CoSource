...
describe('product discovery UI', () => {
  ...
  it('promotes a comparison-selected candidate into WebMCP proposal co-activity without pre-adding it',async()=>{
    ...
    try{render(<App/>);await screen.findByText('WebMCP ready');await userEvent.type(screen.getByLabelText('What are you trying to buy or achieve?'),'A candidate product');const input=screen.getByLabelText('What are you looking for?');await userEvent.type(input,'candidate');fireEvent.submit(input.closest('form')!);await screen.findByText('Human-selected candidate')
      ...
      let goal!:Record<string,unknown>,plan!:Record<string,unknown>;await act(async()=>{goal=await tool('get_purchase_goal').execute({}) as Record<string,unknown>;plan=await tool('get_purchase_plan').execute({}) as Record<string,unknown>});const goalData=goal.data as {id:string;revision:number},planData=plan.data as {id:string;revision:number;lines:unknown[]};expect(planData.lines).toHaveLength(0)
      await act(async()=>{await tool('propose_plan_changes').execute({planId:planData.id,planRevision:planData.revision,goalId:goalData.id,goalRevision:goalData.revision,operations:[{type:'add_retained_product',product:candidate.identity}],reason:'Add the human-selected candidate'})});expect(await screen.findByText('Add the human-selected candidate')).toBeInTheDocument();await act(async()=>{plan=await tool('get_purchase_plan').execute({}) as Record<string,unknown>});expect((plan.data as {revision:number;lines:unknown[]}).lines).toHaveLength(0)
