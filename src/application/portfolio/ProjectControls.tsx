...
export function ProjectControls({portfolio,snapshot}:{portfolio:ProjectPortfolio;snapshot:PortfolioSnapshot}){
 ...
 return <section className="project-controls" aria-label="Saved projects">
  ...
  <details className="project-menu"><summary>Manage projects</summary><div className="project-menu-body"><form onSubmit={event=>{event.preventDefault();void act(portfolio.createProject(newName)).then(ok=>{if(ok)setNewName('')})}}><label htmlFor="new-project-name">New project name</label><div><input id="new-project-name" value={newName} maxLength={100} disabled={busy||locked} onChange={event=>setNewName(event.target.value)}/><button disabled={busy||locked}>Create project</button></div></form><form onSubmit={event=>{event.preventDefault();void act(portfolio.renameActiveProject(rename))}}><label htmlFor="rename-project">Rename active project</label><div><input id="rename-project" value={rename} maxLength={100} disabled={busy||locked} onChange={event=>setRename(event.target.value)}/><button className="secondary" disabled={busy||locked}>Rename</button></div></form></div></details>
