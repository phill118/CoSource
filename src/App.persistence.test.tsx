...
describe('visible persistence lifecycle',()=>{
 ...
 it('keeps a preserved memory workspace visible while recovery and WebMCP remain blocked',()=>{const retry=vi.fn();show({status:'recovery_required',reason:'corrupt',access:'memory_preserved',message:'Your current workspace is preserved in memory.'},retry);expect(screen.getByRole('alert')).toHaveTextContent('Workspace preserved in memory');fireEvent.click(screen.getByRole('link',{name:'Discover'}));fireEvent.click(screen.getByText('Exploratory catalog search'));expect(screen.getByRole('button',{name:'Search products'})).toBeInTheDocument();expect(screen.getByText('WebMCP unavailable in this browser')).toBeInTheDocument();fireEvent.click(screen.getByRole('button',{name:'Retry local storage'}));expect(retry).toHaveBeenCalledOnce()})
