...
Current discovery is compiled through the provider-independent [universal sourcing boundary](../architecture/universal-sourcing.md). The purchasing strategy remains the human-readable product projection, while the canonical source plan records the requirement/revision binding, selected catalogue source, applied constraints, unsupported mandatory and preference intent, and capability warnings. Executable sources with fewer mandatory limitations come first; requester preference and preference coverage distinguish only sources with equivalent mandatory capability, followed by stable source ID. This selects an execution source, never a product or winner.
